...
c(10) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.650 .340 .25 .04],'String','Ankle','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportAnkle(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''ankle''; generatePdf(d,f,pluginFolder,patientFolder,filename);');
c(11) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.650 .290 .25 .04],'String','Knee','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportKnee(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''knee''; generatePdf(d,f,pluginFolder,patientFolder,filename);');
c(12) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.650 .240 .25 .04],'String','Hip','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportHip(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''hip''; generatePdf(d,f,pluginFolder,patientFolder,filename);');

% Generate report and CD
% -------------------------------------------------------------------------
cd(pluginFolder);
d(1) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.650 .040 .3 .09],'String','Generate report','Visible','on','call',...
