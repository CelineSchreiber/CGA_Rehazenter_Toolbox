...
% Initialisation
% =========================================================================
clearvars -except normativeFile patientFolder toolboxFolder reportFolder sessionFolder;
clc;
warning('off','All');
cd(toolboxFolder);
addpath([pwd,'\toolbox\gs']);
pluginFolder = [pwd,'\module\lowerLimb\clinicalReport'];
normFolder = [pwd,'\norm\'];
cd(toolboxFolder);
...
c(1) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    ...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportFirstPage(Patient,Pathology,Treatment,Session,Condition,pluginFolder);filename = ''firstPage''; generatePdf(d,f,pluginFolder,patientFolder,filename);');
c(2) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.050 .390 .25 .04],'String','Gait Parameters','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportGaitParameters(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''gaitParameters''; generatePdf(d,f,pluginFolder,patientFolder,filename);');
c(3) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.050 .340 .25 .04],'String','Ankle','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportAnkle(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''kinematics''; generatePdf(d,f,pluginFolder,patientFolder,filename);');
...
c(4) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    ...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportKinetics(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''kinetics''; generatePdf(d,f,pluginFolder,patientFolder,filename);');
...
c(5) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    ...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportEMG(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''EMG''; generatePdf(d,f,pluginFolder,patientFolder,filename);');
...
d(2) = uicontrol('Style','checkbox','Units','normalized','Value',0,'String','write in pdf','Position',...
