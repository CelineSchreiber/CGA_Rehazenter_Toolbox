...
function f = reportKnee(Patient,Session,Condition,pluginFolder,normFolder,norm)
...
if size(Condition,2) == 1
    ...
    hold on;
    title('flexion/extension (Flex+)','FontWeight','Bold');
    ...
    if ~isempty(Jkinematics.R_Knee_Angle_FE.mean)
        corridor(Jkinematics.R_Knee_Angle_FE.mean,Jkinematics.R_Knee_Angle_FE.std,colorR(1,:));
        plot(Jkinematics.R_Knee_Angle_FE.mean,'Linestyle','-','Linewidth',2,'Color',colorR(1,:));
    end
    if ~isempty(Jkinematics.L_Knee_Angle_FE.mean)
        corridor(Jkinematics.L_Knee_Angle_FE.mean,Jkinematics.L_Knee_Angle_FE.std,colorL(1,:));
        plot(Jkinematics.L_Knee_Angle_FE.mean,'Linestyle','-','Linewidth',2,'Color',colorL(1,:));
    end
    axis tight;
    YL = ylim;
    YL = setAxisLim(YL,-20,60);
    axis([0 100 YL(1) YL(2)]);
    box on;
    igraph = igraph+1;
    
    % Right/Left Knee obliquity
    % ---------------------------------------------------------------------
    axesGraph = axes;
    ...
    set(Graph(igraph),'FontSize',8,'YGrid','on','XTick',[0 25 50 75 100],'YTick',-100:10:100);
    hold on;
    title('Add/Abduction (Add+)','FontWeight','Bold');
    ...
    if ~isempty(Jkinematics.R_Knee_Angle_AA.mean)
        corridor(Jkinematics.R_Knee_Angle_AA.mean,Jkinematics.R_Knee_Angle_AA.std,colorR(1,:));
        plot(Jkinematics.R_Knee_Angle_AA.mean,'Linestyle','-','Linewidth',2,'Color',colorR(1,:));
    end
    if ~isempty(Jkinematics.L_Knee_Angle_AA.mean)
        corridor(Jkinematics.L_Knee_Angle_AA.mean,Jkinematics.L_Knee_Angle_AA.std,colorL(1,:));
        plot(Jkinematics.L_Knee_Angle_AA.mean,'Linestyle','-','Linewidth',2,'Color',colorL(1,:));
    end
    axis tight;
    YL = ylim;
    ...
    if ~isempty(Dynamics.R_Ankle_Moment_FE.mean) | ~isempty(Dynamics.L_Ankle_Moment_FE.mean)
        ...
        set(axesText,'Visible','Off');
        y = y - yincr*3;
        text(0.02,y/pageHeight,'  Cinétique du genou','Color','k','FontWeight','Bold','FontSize',14,'HorizontalAlignment','Center');
        ...
        hold on;
        title('flexion/extension moment (Flex+)','FontWeight','Bold');
        ...
        if ~isempty(Dynamics.R_Knee_Moment_FE.mean)
            corridor(Dynamics.R_Knee_Moment_FE.mean,Dynamics.R_Knee_Moment_FE.std,colorR(1,:));
            plot(Dynamics.R_Knee_Moment_FE.mean,'Linestyle','-','Linewidth',2,'Color',colorR(1,:));
        end
        if ~isempty(Dynamics.L_Knee_Moment_FE.mean)
            corridor(Dynamics.L_Knee_Moment_FE.mean,Dynamics.L_Knee_Moment_FE.std,colorL(1,:));
            plot(Dynamics.L_Knee_Moment_FE.mean,'Linestyle','-','Linewidth',2,'Color',colorL(1,:));
        end
        axis tight;
        YL = ylim;
    ...
    %     axis([0 100 YL(1) YL(2)]);
        ...
        if ~isempty(Dynamics.R_Knee_Power_FE.mean)
            corridor(Dynamics.R_Knee_Power_FE.mean,Dynamics.R_Knee_Power_FE.std,colorR(1,:));
            plot(Dynamics.R_Knee_Power_FE.mean,'Linestyle','-','Linewidth',2,'Color',colorR(1,:));
        end
        if ~isempty(Dynamics.L_Knee_Power_FE.mean)
            corridor(Dynamics.L_Knee_Power_FE.mean,Dynamics.L_Knee_Power_FE.std,colorL(1,:));
            plot(Dynamics.L_Knee_Power_FE.mean,'Linestyle','-','Linewidth',2,'Color',colorL(1,:));
        end
        axis tight;
        YL = ylim;
    ...
    if ~isempty(Condition(icondition).Trial(itrial).LowerLimb.EMG)
        ...
        for j = 1:length(names)/3
            % Get recorded EMG 
            if ~isempty(strfind(names{3*j-2},'vastus_medialis_Raw')) | ~isempty(strfind(names{3*j-2},'rectus_femoris_Raw')) | ~isempty(strfind(names{3*j-2},'semitendinosus_Raw'))
