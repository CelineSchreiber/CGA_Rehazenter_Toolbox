...
for i = 1:length(Session.conditions)
    ...
    staticMarkers = btkGetMarkers(btk2);    
    % ---------------------------------------------------------------------
    % Video
    % ---------------------------------------------------------------------
    ...
    for j = 1:length(Session.Trial)
        if strcmp(Session.Trial(j).condition,Condition(i).name)
            ...
            % Head/Trunk limb kinematic chain
            % -------------------------------------------------------------
            Condition(i).Trial(k).HeadTrunk = [];
            if Session.Trial(j).kinematics.headTrunk == 1
                disp('      Head/Trunk/Pelvis - posturo');
                % Set body segments for kinematics
                [Segment,Vmarker,btk2] = ...
                    setTrialSegment_kinematics_headTrunk(Condition(i),Marker,btk2);
                % Compute segment kinematics
                [Segment,btk2] = computeSegmentKinematics_headTrunk(Segment,btk2);
                % Compute joint kinematics
                [Joint,btk2] = computeJointKinematics_headTrunk(Segment,btk2); 
                % Store data in Condition (keep only intra cycle data)
                Condition(i).Trial(k).HeadTrunk = ...
                    exportCondition_headTrunk(Condition(i).Trial(k).HeadTrunk,Segment,Joint,Marker,Vmarker,Event,fMarker);
            end
...
%             writeConditionC3d(Session,Condition(i),btk2,Session.Trial(j).filename,k);
    ...
    cd(c3dFolder);
    filename = [Patient.lastname,'_',Patient.firstname,'_',regexprep(Patient.birthdate,'/',''),'_',Condition(i).name,'_',regexprep(Session.date,'/',''),'.c3d'];
%     system(['ren temp.c3d ',filename]);
%     system(['C:\Users\celine.schreiber\Downloads\Mokka\Mokka.exe -c ',toolboxFolder,'\LWBM_model.mvc -p ',filename]);
    cd(toolboxFolder);
end
